...
router.post('/', async (req, res) => {
  try {
    const { name, description, price, brand, images: pictures } = req.body
...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    const similar = await Product.find({ brand: product.brand }).limit(5)
...
router.get('/brand/:brand', async (req, res) => {
  ...
  try {
    ...
    if (brand == 'all') {
      ...
      products = await Product.find({ brand }).sort(sort)
    }
    res.status(200).json(products)
  } catch (e) {
    res.status(400).send(e.message)
